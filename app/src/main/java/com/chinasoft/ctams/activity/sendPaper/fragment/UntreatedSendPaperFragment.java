 ...
 */
public class UntreatedSendPaperFragment extends BaseFragment {
    private final String URL_SEND_PAPER_LIST = "bizSubmittedInfo/listBizReceiveinfoCon";
    @Bind(R.id.lv_untreated_send_paper)
    XListView lvUntreatedSendPaper;
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (lvUntreatedSendPaper!=null)
            lvUntreatedSendPaper.stopLoadMore();
    ...
    public int getLayoutId() {
        return R.layout.fragment_untreated_send_paper_list;
    }
    ...
    private void init() {
        ...
        lvUntreatedSendPaper.setAdapter(lvSendPaperAdapter);
        lvUntreatedSendPaper.setPullRefreshEnable(true);
        lvUntreatedSendPaper.setPullLoadEnable(true);
        organizationId = SharedPreferencesHelper.getInstance().getOrganizationId(getActivity());
        ServerModel.getInstance(getActivity()).getSendPaperByType(handler, URL_SEND_PAPER_LIST, "1000032", organizationId,1,getActivity());
    }

    private void initData() {
        lvUntreatedSendPaper.setXListViewListener(new XListView.IXListViewListener() {
            @Override
            ...
            public void onLoadMore() {
                pageNo++;
                isFresh=false;
                ServerModel.getInstance(getActivity()).getSendPaperByType(handler, URL_SEND_PAPER_LIST, "1000032", organizationId,pageNo,getActivity());
            }
        });
        lvUntreatedSendPaper.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
