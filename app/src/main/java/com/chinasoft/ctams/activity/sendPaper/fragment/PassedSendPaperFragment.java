 ...
 */
public class PassedSendPaperFragment extends BaseFragment {
    private final String URL_SEND_PAPER_LIST = "bizSubmittedInfo/listBizReceiveinfoCon";

    @Bind(R.id.lv_passed_send_paper)
    XListView lvPassedSendPaper;
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (dialog!=null)
                dialog.dismiss();
            if (lvPassedSendPaper!=null)
            lvPassedSendPaper.stopLoadMore();
    ...
    public int getLayoutId() {
        return R.layout.fragment_passed_send_paper_list;
    }
    ...
    private void init() {
        ...
        lvPassedSendPaper.setAdapter(lvSendPaperAdapter);
        lvPassedSendPaper.setPullRefreshEnable(false);
        lvPassedSendPaper.setPullLoadEnable(true);
        organizationId = SharedPreferencesHelper.getInstance().getOrganizationId(getActivity());
        ServerModel.getInstance(getActivity()).getSendPaperByType(handler, URL_SEND_PAPER_LIST, "1000031", organizationId, 1,getActivity());
        if (dialog==null){
    ...
    private void initData() {
        lvPassedSendPaper.setXListViewListener(new XListView.IXListViewListener() {
            @Override
            ...
            public void onLoadMore() {
                isFresh=false;
                pageNo++;
                ServerModel.getInstance(getActivity()).getSendPaperByType(handler, URL_SEND_PAPER_LIST, "1000031",organizationId, pageNo,getActivity());
            }
        });
        lvPassedSendPaper.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
