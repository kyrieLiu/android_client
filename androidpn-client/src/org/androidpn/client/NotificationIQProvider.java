	...
	public IQ parseIQ(XmlPullParser parser) throws Exception {
		...
		for (boolean done = false; !done;) {
			...
			if (eventType == 2) {
				...
				if ("sendUser".equals(parser.getName())){
					notification.setSendUser(parser.nextText());
				}
				if ("sendTime".equals(parser.getName())){
					notification.setSendTime(parser.nextText());
				}
				if ("peopleName".equals(parser.getName())){
					notification.setPeopleName(parser.nextText());
				}
			} else if (eventType == 3
